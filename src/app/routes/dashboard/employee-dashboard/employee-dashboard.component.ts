...
export class EmployeeDashboardComponent implements OnInit, OnDestroy {
  ...
  private loadAttendance(): void {
    ...
    forkJoin({
      current: this.attendanceService.getMySummary({
        month: curMonth,
        year: curYear,
      }),
      previous: this.attendanceService.getMySummary({
        month: prevMonth,
        year: prevYear,
      }),
    }).subscribe({
      next: ({ current, previous }) => {
        ...
        this.targetValues = [
          current.presentDays ?? 0,
          current.lateDays ?? 0,
          current.absentDays ?? 0,
          current.halfDays ?? 0,
          current.leaveDays ?? 0,
        ];
  ...
  private computeRates(
    ...
    const rate = (d: AttendanceSummary) => {
      const worked =
        (d.presentDays ?? 0) + (d.lateDays ?? 0) + (d.halfDays ?? 0);
    ...
    const configs = [
      {
        label: 'PRESENT',
        valueColor: '#059669',
        gradient: 'linear-gradient(180deg,#6ee7b7 0%,#059669 100%)',
        lightBg: '#ecfdf5',
        animDelay: '0s',
      },
      {
        label: 'LATE',
        valueColor: '#d4920a',
        gradient: 'linear-gradient(180deg,#fde68a 0%,#d4920a 100%)',
        lightBg: '#fffbeb',
        animDelay: '0.12s',
      },
      {
        label: 'ABSENT',
        valueColor: '#e11d48',
        gradient: 'linear-gradient(180deg,#fda4af 0%,#e11d48 100%)',
        lightBg: '#fff1f2',
        animDelay: '0.24s',
      },
      {
      ...
      },
      {
        label: 'LEAVE',
        valueColor: '#0891b2',
        gradient: 'linear-gradient(180deg,#7dd3fc 0%,#0891b2 100%)',
        lightBg: '#ecf8ff',
        animDelay: '0.48s',
      },
    ];
