import { fakeAsync, discardPeriodicTasks, flushMicrotasks, TestBed, tick } from '@angular/core/testing';
...
describe('AuthService', () => {
  ...
  it('should log in successful and get user info', () => {
    ...
    const userSub   = authService.user().pipe(skip(1)).subscribe({
    ...
    httpMock
      .expectOne(req => req.url.includes('auth/login'))
      .flush({ token: makeJwt(3600), tokenType: 'bearer', expiresIn: 3600, user });
  ...
  it('should refresh token when access_token is valid', fakeAsync(() => {
    ...
    flushMicrotasks();
    tick(0);
    // Use match() — won't throw if /me hasn't fired yet in fakeAsync
    httpMock.match('/api/auth/me').forEach(req => req.flush(user));

    tick(4000);
    httpMock
      .match((req: HttpRequest<any>) => req.url === '/api/auth/refresh' && !req.body?.refresh_token)
      [0]?.flush(token);
  ...
  it('should clear token when refresh returns 401', fakeAsync(() => {
    ...
    httpMock
      .match((req: HttpRequest<any>) =>
        req.url === '/api/auth/refresh' && req.body?.refresh_token === 'foo'
      )[0]?.flush({}, { status: 401, statusText: 'Unauthorized' });
  ...
  // Not wrapped in fakeAsync — no tick() needed, synchronous BehaviorSubject chain
  it('should call /me only once when subscribed twice', () => {
    authService.change().subscribe();
    authService.change().subscribe();

    tokenService.set(token);
    // shareReplay(1) → single /me call even with 2 subscribers
    httpMock.expectOne('/api/auth/me').flush(user);

    expect(authService.check()).toBeTrue();
    tokenService.ngOnDestroy();  // stop the internal refresh timer
  });
});